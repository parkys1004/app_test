...
const ConceptTab = ({ project, onUpdate, legibilityMode }: { project: Project, onUpdate: (u: Partial<Project>) => void, legibilityMode: boolean }) => {
  ...
  const usageCount = project.themePackGenerationCount || 0;
  const MAX_PACK_GENERATIONS = 1; // Changed from 3 to 1
  const isLimitReached = usageCount >= MAX_PACK_GENERATIONS;
  ...
  const generateThemePacks = async () => {
    if (isLimitReached) {
        alert('추천 팩 생성은 프로젝트당 최대 1회까지만 가능합니다.');
        return;
    }
    ...
    try {
        ...
        // Save to project state to persist across tabs, and increment count
        onUpdate({ 
            generatedThemePacks: data,
            themePackGenerationCount: usageCount + 1
        });
    } catch (e) {
        console.error(e);
      ...
      <div style={{ marginBottom: '40px', padding: '20px', backgroundColor: '#111827', borderRadius: '12px', border: '1px solid #e11d48' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px', flexWrap: 'wrap', gap: '15px' }}>
              ...
              <button 
                ...
                disabled={loadingPacks || isLimitReached}
                style={{ 
                    padding: '10px 20px', 
                    backgroundColor: isLimitReached ? '#374151' : '#e11d48', 
                    color: isLimitReached ? '#9ca3af' : 'white', 
                    border: 'none', 
                    borderRadius: '8px', cursor: isLimitReached ? 'not-allowed' : 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' 
                }}
                title={isLimitReached ? "프로젝트당 최대 1회까지 생성 가능합니다." : ""}
              >
                {loadingPacks ? 'AI 추천 생성 중...' : (
                    <><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>magic_button</span> 추천 팩 생성 ({usageCount}/{MAX_PACK_GENERATIONS})</>
                )}
              </button>
          </div>
